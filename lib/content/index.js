...
Content.prototype.fetch = function (page, callback) {
	...
	if (page) {
		...
		this.AppContent.findOne({ key: page }).then((result) => {
			return callback(null, content.pages[page].populate(result ? result.content.data : {}));
		}).catch(callback);

	} else {

		this.AppContent.find().then((results) => {
			var data = {};
...
Content.prototype.store = function (page, content, callback) {
	...
	this.AppContent.findOne({ key: page }).then((doc) => {
		if (doc) {
			if (doc.content) {
		...
		doc.save().then((data) => callback(null, data)).catch(callback);
