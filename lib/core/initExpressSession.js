...
module.exports = function initExpressSession (mongoose) {
	...
	if (typeof sessionStore === 'function') {
		...
		var SessionStore = safeRequire(sessionStore, this.get('session store') + ' as a `session store` option');

		sessionStoreCreate = (client) => {
			sessionStoreOptions.client = client;
			sessionOptions.store = SessionStore.create(sessionStoreOptions);
	...
	this.sessionStoreCreate = sessionStoreCreate;
