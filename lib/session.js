...
var doSignin = async function (lookup, req, res, onSuccess, onFail) {
	if (!lookup) {
	...
	if (typeof lookup.email === 'string' && typeof lookup.password === 'string') {
		...
		try {
			const user = await User.model.findOne({ email: emailRegExp });
			if (user) {
...
exports.persist = async function (req, res, next) {
	...
	if (keystone.get('cookie signin') && !req.session.userId && req.signedCookies['keystone.uid'] && req.signedCookies['keystone.uid'].indexOf(':') > 0) {
		...
		try {
			const user = await User.model.findById(req.session.userId);
			req.user = user;
			next();
		} catch (err) {
			return next(err);
